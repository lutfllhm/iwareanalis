...
interface PelangganRow {
  idPelanggan: string;
  idKaryawanDefaultPenjual: string;
  namaDefaultPenjual: string;
  idKaryawanTenagaPenjualKedua: string;
  nama: string;
  kategoriPelanggan: string;
  nonAktif: boolean;
  kotaPengiriman: string;
  provinsiPengiriman: string;
  tglJamPembuatan: string | null;
  alamatLengkapPengiriman: string;
}
...
export default function DaftarPelangganPage() {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['accurateDaftarPelanggan', page, limit, q],
    queryFn: async () => {
      const res = await api.get('/report/daftar-pelanggan', {
        params: { page, limit, q },
      });
      return res.data;
    },
  ...
  const rows: PelangganRow[] = data?.data || [];
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Menampilkan daftar pelanggan langsung dari Accurate Online
            </p>
          </div>
        </div>
      ...
      <div className="card-elevated bg-card border border-border rounded-xl p-4">
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-end">
          <div className="flex-1 flex items-center gap-2 bg-background border border-border rounded-xl px-3 py-2">
            ...
            <input
              type="text"
              placeholder="Cari ID pelanggan, nama, kategori..."
              value={searchInput}
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="bg-muted/50 border-b border-border">
                  <th className="px-4 py-3 text-[11px] font-bold text-muted-foreground uppercase tracking-wider whitespace-nowrap">ID Pelanggan</th>
                  <th className="px-4 py-3 text-[11px] font-bold text-muted-foreground uppercase tracking-wider whitespace-nowrap">ID Karyawan Default Penjual</th>
                  <th className="px-4 py-3 text-[11px] font-bold text-muted-foreground uppercase tracking-wider whitespace-nowrap">ID Karyawan Tenaga Penjual Kedua</th>
                  <th className="px-4 py-3 text-[11px] font-bold text-muted-foreground uppercase tracking-wider">Nama</th>
                  <th className="px-4 py-3 text-[11px] font-bold text-muted-foreground uppercase tracking-wider whitespace-nowrap">Kategori Pelanggan</th>
                  ...
                  <th className="px-4 py-3 text-[11px] font-bold text-muted-foreground uppercase tracking-wider whitespace-nowrap">Kota Pengiriman</th>
                  <th className="px-4 py-3 text-[11px] font-bold text-muted-foreground uppercase tracking-wider whitespace-nowrap">Provinsi Pengiriman</th>
                  ...
                  <th className="px-4 py-3 text-[11px] font-bold text-muted-foreground uppercase tracking-wider whitespace-nowrap">Nama Default Penjual</th>
                  <th className="px-4 py-3 text-[11px] font-bold text-muted-foreground uppercase tracking-wider">Alamat Lengkap Pengiriman</th>
                </tr>
              </thead>
                ...
                {rows.length === 0 ? (
                  ...
                  rows.map((row) => (
                    <tr key={row.idPelanggan} className="hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-4 py-3 text-xs text-foreground whitespace-nowrap">
                        {row.provinsiPengiriman || '-'}
                      </td>
                      <td className="px-4 py-3 text-xs text-foreground whitespace-nowrap">
                        {formatTanggal(row.tglJamPembuatan)}
                      </td>
                      <td className="px-4 py-3 text-xs text-foreground whitespace-nowrap">
                        {row.namaDefaultPenjual || '-'}
                      </td>
                      <td className="px-4 py-3 text-xs text-muted-foreground max-w-xs truncate">
                        {row.alamatLengkapPengiriman || '-'}
                      </td>
                    </tr>
                  ))
                )}
